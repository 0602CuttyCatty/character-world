    ...
    <div style={{ position:"fixed", left:x-40, top:y-40, width:80, height:80, pointerEvents:"none", zIndex:9999 }}>
      {["✨","⭐","💫","🌟","✨","⭐","💫"].map((s,i) => (
        <span key={i} style={{
          ...
          animation:`burst-out 1s ease-out ${i*.06}s forwards`,
...
export default function Page() {
  const [tab, setTab]           = useState<"gallery"|"enhance">("gallery");
  const [enhSubTab, setEnhSub]  = useState<"enhance"|"shop">("enhance");
  ...
  const [result, setResult]         = useState<null|"break"|"protected">(null);
  ...
  // ─────────────────────────────────────────
  //  강화 레벨별 이미지 (0강 ~ 20강)
  //  → public/enhance/ 폴더에 이미지 넣고 경로 수정하세요
  //  → null 이면 기본 배경색 표시
  // ─────────────────────────────────────────
  const LEVEL_IMAGES: (string | null)[] = [
    ...
    "/enhance/lv8.png",   // 8강
    "/enhance/lv9.png",   // 9강
    "/enhance/lv10.png",  // 10강
    "/enhance/lv11.png",  // 11강
    "/enhance/lv12.png",  // 12강
    "/enhance/lv13.png",  // 13강
    "/enhance/lv14.png",  // 14강
    "/enhance/lv15.png",  // 15강
    "/enhance/lv16.png",  // 16강
    "/enhance/lv17.png",  // 17강
    "/enhance/lv18.png",  // 18강
    "/enhance/lv19.png",  // 19강
    "/enhance/lv20.png",  // 20강
  ];
  ...
  // ── 상점: 워프권
  const useWarp = (item: typeof WARP_ITEMS[0]) => {
  ...
  const handleAddChar = () => {
    ...
    setGallery(prev => [...prev, { id:Date.now(), name:newName.trim(), emoji:emojis[i%10], rarity:"common", color:colors[i%6], imageUrl:null }]);
    setNewName(""); setAddOpen(false);
  };

  const bgParticles = Array.from({length:14},(_,i)=>({
    width:`${6+(i*3.7)%10}px`, height:`${6+(i*3.7)%10}px`,
    left:`${(i*17+5)%100}%`, top:`${(i*23+10)%100}%`,
    background:["#FFD6E0","#FFE8CC","#D4F1F4","#E8D5FF","#FFEAA7","#DFE6E9"][i%6],
    animationDelay:`${(i*.6)%4}s`, animationDuration:`${3+(i%3)}s`, opacity:.4,
  }));
    ...
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#FFF0F5 0%,#F0F4FF 50%,#F5FFF0 100%)",fontFamily:"'Nunito','Noto Sans KR',sans-serif",position:"relative",overflow:"hidden"}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        @keyframes success-jump{0%,100%{transform:translateY(0) scale(1);}30%{transform:translateY(-24px) scale(1.13);}60%{transform:translateY(-10px) scale(1.07);}}
      ...
      <div style={{display:"flex",justifyContent:"center",gap:10,marginBottom:22}}>
        {[{key:"gallery",label:"🖼️ 갤러리"},{key:"enhance",label:"⚔️ 강화하기"}].map(t=>(
          <button key={t.key} className={`tab-btn${tab===t.key?" active":""}`}
            onClick={()=>setTab(t.key as "gallery"|"enhance")}
            style={{
            ...
            {gallery.map((c,i) => {
              ...
              return (
                <div key={c.id} className="char-card"
                  style={{animationDelay:`${i*.07}s`,background:`linear-gradient(145deg,white,${c.color}18)`,border:`2.5px solid ${c.color}66`,borderRadius:22,padding:"18px 12px 14px",textAlign:"center",boxShadow:`0 4px 16px ${c.color}28`,position:"relative",overflow:"hidden"}}
                  ...
                  <div style={{width:70,height:70,borderRadius:"50%",
                    background:c.imageUrl?`url(${c.imageUrl}) center/cover`:`radial-gradient(circle at 35% 35%,${c.color}44,${c.color}BB)`,
                    margin:"0 auto 10px",display:"flex",alignItems:"center",justifyContent:"center",fontSize:30,
                    boxShadow:`0 4px 12px ${c.color}55`,border:`3px solid ${c.color}66`,
                    transition:"transform .2s",transform:hoverCard===c.id?"scale(1.1) rotate(7deg)":"scale(1)"}}>
                    {!c.imageUrl && c.emoji}
                  </div>
      ...
      {tab==="enhance" && (
        <div style={{maxWidth:440,margin:"0 auto",padding:"0 14px 60px",animation:"slide-up .4s ease"}}>
          ...
          <div style={{display:"flex",background:"white",borderRadius:16,padding:4,marginBottom:14,boxShadow:"0 2px 10px rgba(200,100,180,.1)",border:"1.5px solid #F0D0E8",gap:4}}>
            {[{key:"enhance",label:"⚔️ 강화하기"},{key:"shop",label:"🏪 상점"}].map(t=>(
              <button key={t.key} className="sub-tab-btn"
                onClick={()=>setEnhSub(t.key as "enhance"|"shop")}
                style={{
                  flex:1, padding:"9px 0", borderRadius:12, fontSize:13, fontWeight:800, fontFamily:"inherit",
                  background:enhSubTab===t.key?"linear-gradient(135deg,#FF6B9D,#C44DFF)":"transparent",
                  color:enhSubTab===t.key?"white":"#C08BAB",
                  boxShadow:enhSubTab===t.key?"0 3px 10px rgba(196,77,255,.3)":"none",
                }}>
                {t.label}
              </button>
            ))}
          ...
          {enhSubTab==="enhance" && (
            <>
                ...
                <div style={{padding:"20px 18px 14px",display:"flex",flexDirection:"column",alignItems:"center"}}>
                  <div style={{position:"relative",marginBottom:14}}>
                    {enhLv>=6 && (
                      <div style={{position:"absolute",inset:-24,borderRadius:"50%",
                        background:`radial-gradient(circle,${ENHANCE_COLOR(enhLv)}44 0%,transparent 68%)`,
                        animation:"glow-pulse 1.4s ease-in-out infinite",pointerEvents:"none"}}/>
                    )}
                    {enhLv>=10 && <>
                    ...
                    <div
                      ref={imgRef}
                      className={
                        result==="break"     ? "img-break"     :
                        result==="protected" ? "img-protected" : ""
                      }
                      style={{
                    ...
                    <div style={{
                      ...
                      background:enhLv===0?"#e8e8e8":`linear-gradient(135deg,${ENHANCE_COLOR(enhLv)}CC,${ENHANCE_COLOR(enhLv)})`,
                      color:enhLv===0?"#888":"white",
                      fontSize:14,fontWeight:900,padding:"3px 16px",borderRadius:20,
                      boxShadow:enhLv>=10?`0 0 14px 4px ${ENHANCE_COLOR(enhLv)}88`:`0 2px 8px ${ENHANCE_COLOR(enhLv)}55`,
                      border:"2.5px solid white",whiteSpace:"nowrap",
                    }}>
                  ...
                  {canSell && (
                    <button onClick={handleSell}
                      ...
                      onMouseEnter={e=>{(e.currentTarget as HTMLButtonElement).style.cssText+="background:#FFF0F8;border-color:#FF6B9D;color:#FF6B9D;"}}
                      onMouseLeave={e=>{(e.currentTarget as HTMLButtonElement).style.cssText+="background:none;border-color:#F0D0E0;color:#C08BAB;"}}>
                  ...
                  <div style={{padding:"0 18px 16px"}}>
                    <div style={{background:"#F8F0F8",borderRadius:14,padding:"11px 13px"}}>
                      ...
                      <div style={{height:9,background:"#EDD8E8",borderRadius:9,overflow:"hidden",marginBottom:6}}>
                        <div style={{height:"100%",borderRadius:9,width:`${curRow.rate}%`,transition:"width .5s",
                          background:curRow.rate>=60?"linear-gradient(90deg,#00B894,#55EFC4)":curRow.rate>=30?"linear-gradient(90deg,#FDCB6E,#E17055)":"linear-gradient(90deg,#FF7675,#D63031)",
              ...
              {/* 결과 배너 (파괴 / 방지권만. 성공은 배너 없음) */}
              {result && (
                <div style={{marginBottom:12,borderRadius:20,overflow:"hidden",animation:"result-pop .4s cubic-bezier(.34,1.56,.64,1)",
                  boxShadow:result==="break"?"0 6px 22px rgba(255,50,50,.25)":"0 6px 22px rgba(108,92,231,.2)"}}>
                  ...
                  <button onClick={()=>setResult(null)}
                    style={{width:"100%",padding:"11px",border:"none",background:"rgba(0,0,0,.07)",fontFamily:"inherit",fontWeight:800,fontSize:12,cursor:"pointer",
                      color:result==="break"?"#FF8080":"#9B8BFF"}}>
                    확인
                  </button>
                </div>
              )}
              ...
              {enhLv < 20 && (
                <button className="action-btn" onClick={handleEnhance} disabled={!canEnhance || !!result}
                  style={{
                    width:"100%", padding:"17px", borderRadius:18, fontSize:17, fontWeight:900, fontFamily:"inherit",
                    background:(!canEnhance||!!result)?"#EEE8F4":"linear-gradient(135deg,#FF6B9D,#C44DFF)",
                    color:(!canEnhance||!!result)?"#BBA0CC":"white",
                    boxShadow:(canEnhance&&!result)?"0 9px 24px rgba(196,77,255,.4)":"none",
              ...
              <button onClick={()=>setShowTable(p=>!p)}
                ...
                onMouseEnter={e=>{(e.currentTarget as HTMLButtonElement).style.cssText+="background:#FFF0F8;border-color:#FF6B9D;"}}
                onMouseLeave={e=>{(e.currentTarget as HTMLButtonElement).style.cssText+="background:white;border-color:#F0D0E0;"}}>
          ...
          {enhSubTab==="shop" && (
            <div style={{animation:"slide-up .3s ease"}}>
              ...
              {/* 방지권 */}
              <div style={{background:"white",borderRadius:20,padding:"16px 16px",marginBottom:12,border:"2px solid #C44DFF28",boxShadow:"0 4px 16px rgba(108,92,231,.09)"}}>
              ...
              {/* 워프권 */}
              <div style={{background:"white",borderRadius:20,padding:"16px 16px",border:"2px solid #FFD70028",boxShadow:"0 4px 16px rgba(255,180,0,.07)"}}>
                  ...
                  {WARP_ITEMS.map(item => {
                    ...
                    return (
                      <div key={item.targetLv}
                        style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,padding:"11px 13px",borderRadius:14,
                          background:canBuy?"linear-gradient(135deg,#FFFBE8,#FFF5CC)":"#F8F8F8",
                          border:`1.5px solid ${canBuy?"#FFD70055":"#EEE"}`}}>
                        <div style={{display:"flex",alignItems:"center",gap:10}}>
                          <div style={{width:42,height:42,borderRadius:12,
                            background:canBuy?"linear-gradient(135deg,#FFD700,#FF8C00)":"#DDD",
                            display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,
                            boxShadow:canBuy?"0 3px 10px rgba(255,180,0,.38)":"none"}}>
                            {item.emoji}
                          </div>
                          <div>
                        ...
                        <button className="action-btn" onClick={()=>useWarp(item)} disabled={!canBuy}
                          style={{padding:"8px 14px",borderRadius:11,border:"none",flexShrink:0,
                            background:canBuy?"linear-gradient(135deg,#FFD700,#FF8C00)":"#EEE",
                            color:canBuy?"white":"#AAA",fontFamily:"inherit",fontWeight:800,fontSize:12,cursor:"pointer",
                            boxShadow:canBuy?"0 3px 10px rgba(255,180,0,.35)":"none"}}>
                          워프!
                        </button>
                      </div>
                    );
